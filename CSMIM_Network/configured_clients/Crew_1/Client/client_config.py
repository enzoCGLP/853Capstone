...
#################### CA Cert Info ####################
ca_certs_file = "CSMIM_Network/Client_v_1_6_raw/certs/ca-root-cert.crt"  # File location of CA cert
...
is_executable = "no" # "no" if client is not executable
can_execute = "yes" # "no" if client can not execute on another client
can_read = "yes" # "no" if client can not read from a topic
can_send = "no" # "no" if client can not send to a topic
...
reg_payload = {
    ...
    "types": [
    { "id": "csmim.obj.station.1", # Object ID to be registered
      ...
      { 
          ...
          "mode": "r", # Mode type of resource i.e.: execute: 'x', read: 'r', and write:'w'
    ...
    "objects": [ # type list
    { 
      "path": "crew/station/1/0", # Path of object to be registered. Must be an unused path.
      "type": "csmim.obj.station.1", # The object id to be registered. (From above)
      ...
      { 
        "id": "status", # Resource ID (From above)
        "mode": "r", # Mode type of resource i.e.: execute: 'x', read: 'r', and write:'w' (From above)
