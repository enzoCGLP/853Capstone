##################################################################################################################################################
# CSMIM FUNCTIONS: READ, SEND, EXECUTE, REGISTER, RESPOND*, and Execute subscribe
# Version: 1.6
...
# * RESPOND is not currently implemented for EXECUTE.
...
###### CSMIM SUBSCRIBE TO TOPIC ######
# To SUBSCRIBE to client's executable 

def csmim_subscribe(client, topic):
    ###### Uncomment for Debug ######
    #client.on_subscribe = on_subscribe
    #client.on_message = decode_cbor_message
    ###### Set custom user properties for SUBSCRIBE ######   
...
###### CSMIM REGISTER ######
def csmim_register (client, topic, payload, user_name):
  ###### Uncomment for Debug ######
  #client.on_publish = on_publish
  ###### Set custom user properties for PUBLISH ######
    properties = Properties(PacketTypes.PUBLISH)
    properties.UserProperty=[("respond","0"), ("username", f"{user_name}")]  
    client.publish(topic, payload, qos=0, retain=True, properties=properties)

###### CSMIM RESPONSE ######
def csmim_response(client, topic, payload):
        properties = Properties(PacketTypes.PUBLISH)
        client.publish(topic, payload, qos=0, retain=True, properties=properties)
