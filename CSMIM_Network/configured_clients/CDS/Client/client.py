###################################################################################################################################################
# CLIENT: Registers with CDS and Connects to MQTT broker, calls CSMIM functions, and disconnects client from MQTT broker.
...
csmim_functions.csmim_subscribe(cur_client, client_config.response_topic)

###### PUBLISH TO REGISTER TOPIC ######
encoded_payload = topic_functions.encode_cbor_data(client_config.reg_payload)
csmim_functions.csmim_register(cur_client, client_config.reg_topic, encoded_payload, client_config.user_name)
time.sleep(1)
###### Check if REGISTER was successful/ disconnect if not ######
if client_config.is_registered == True:
...
###### Check if client is executable and subscribes to its executable topic ######
    if client_config.is_executable.lower() == "yes":
        ...
        csmim_functions.csmim_subscribe(cur_client, client_config.topic_1)
